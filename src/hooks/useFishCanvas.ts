// Canvas描画ロジック共通化フック
// FishPreviewから抽出したCanvas描画機能をhookとして提供

import { useCallback } from 'react';
import type { FishDesign } from '../types/common.types';
import type { CanvasRenderResult, FishCanvasConfig } from '../types/i2i.types';

export default function useFishCanvas() {
  // 色の明度を調整するヘルパー関数
  ...
  }, [drawBody, drawBodyPattern, drawFins, drawScales, drawEyes, drawMouth, drawAccessories]);

  // FishDesignからCanvas画像を生成する関数
  const generateCanvasImage = useCallback((
    fishDesign: FishDesign, 
    config: FishCanvasConfig = { width: 800, height: 600 }
    ...
    return new Promise((resolve) => {
      try {
        ...
        // 魚を描画
        drawFish(ctx, fishDesign, config.width, config.height);
        ...
        resolve({
          success: false,
          error: `Canvas generation failed: ${error}`,
          dimensions: { width: config.width, height: config.height }
        });
      }
    });
